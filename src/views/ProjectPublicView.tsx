    ...
    <div className="pb-20 space-y-0 max-w-3xl mx-auto">
      ...
      <div className="flex gap-1 border-b border-white/8 overflow-x-auto no-scrollbar mb-6">
        {[
          ...
          { key: 'invest' as const, labelFR: 'Certification', labelEN: 'Certification' },
        ].map(tab => (
        ...
        <div className="space-y-5">
          ...
          <div className="bg-accent-gold/5 border border-accent-gold/20 rounded-2xl p-5 space-y-3">
            ...
            <p className="text-sm text-on-surface-variant/70 leading-relaxed">
              {T(
                `Le score LYA et l'indexation reflètent l'authenticité réglementaire, juridique et artistique de ce projet. Ils sont basés sur le niveau d'audit certifié par des validateurs indépendants et évoluent selon le respect des jalons du projet.`,
                `The LYA score and indexing reflect the regulatory, legal, and artistic authenticity of this project. They are based on the audit level certified by independent validators and evolve according to milestone compliance.`
              )}
            </p>
            <div className="grid grid-cols-3 gap-3 pt-1">
              {[
                { l: 'LYA SCORE', v: `${project.totalScore}/1000`, c: 'text-primary-cyan' },
                { l: T('Audit', 'Audit'), v: T('Vérifié', 'Verified'), c: 'text-emerald-400' },
                { l: T('Conformité', 'Compliance'), v: '100%', c: 'text-primary-cyan' },
              ].map((s,i) => (
          ...
          {project.status !== 'LIVE' && (
            <div className="bg-rose-500/8 border border-rose-500/20 rounded-2xl p-4 flex items-center gap-3">
              ...
              <p className="text-sm text-rose-400 font-black">{T('Ce projet n\'accepte pas de nouvelles évaluations actuellement.','This project is not accepting new audits right now.')}</p>
            </div>
          )}

          {/* CTA */}
          {user ? (
            <button onClick={() => project.status === 'LIVE' ? onViewChange('REGISTRY') : null}
              ...
              <Shield size={20}/>
              {project.status === 'LIVE' ? T('Consulter le Registre', 'Consult the Registry') : T('Non disponible actuellement', 'Currently unavailable')}
            </button>
          ) : (
            <div className="space-y-3">
              <div className="bg-surface-low/40 border border-white/10 rounded-2xl p-5 space-y-4">
                <div className="flex items-start gap-3">
                  ...
                  <div>
                    <p className="text-sm font-black text-on-surface">{T('Rejoignez LinkYourArt pour vérifier ce projet', 'Join LinkYourArt to verify this project')}</p>
                    <p className="text-xs text-on-surface-variant/60 mt-1">{T('Créez un compte gratuit et accédez au registre officiel d\'audit et d\'évaluation.', 'Create a free account and access the official audit and evaluation registry.')}</p>
                  </div>
                </div>
              </div>
          ...
          <p className="text-xs text-on-surface-variant/25 text-center leading-relaxed">
            {T('⚠ Le score LYA est fourni à titre indicatif selon un audit technique, juridique et artistique rigoureux. LinkYourArt est un registre d\'évaluation et de certification, et ne propose aucun service financier réglementé.', '⚠ The LYA score is provided as an indicator based on a rigorous technical, legal, and artistic audit. LinkYourArt is an evaluation and certification registry, and does not offer any regulated financial services.')}
          </p>
        </div>
      )}
