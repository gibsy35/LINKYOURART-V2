...
export const ExchangeView: React.FC<ExchangeViewProps> = ({
  ...
  return (
    <div className="space-y-12 pb-20 relative min-h-screen">
      <LockedOverlay
        title={t('REGULATORY COLD STORAGE ACTIVE (LYA v3.0)', 'GEL RÉGLEMENTAIRE DU PROTOCOLE (LYA v3.0)')}
        description={t(
          'In compliance with LYA Protocol v3.0 (Regulatory-First Architecture), negotiable secondary market transactions (including exchange orders and instant settlement) are in a regulatory intermission phase. The terminal currently operates on static, sandboxed datasets for objective certification, LYA Score evaluation, and professional audit indexing purposes only.',
          "Conformément à la version v3.0 du protocole LYA (Regulatory-First), les transactions négociables du marché secondaire (incluant carnets d'ordres et règlements instantanés) sont suspendues durant cette phase de gel réglementaire. Le terminal fonctionne actuellement sur des données statiques pour la certification objective, l'évaluation du LYA Score et l'indexation d'audits professionnels."
        )}
        actionLabel={t('VIEW ACTIVE CERTIFICATIONS', 'VOIR LES CERTIFICATIONS ACTIVES')}
        onAction={() => {
          window.dispatchEvent(new CustomEvent('changeView', { detail: 'REGISTRY' }));
        }}
      />
      <PageHeader
      ...
      <div className="mx-2 md:mx-4 p-6 border-l-4 border-accent-gold bg-accent-gold/5 rounded-r-2xl border border-white/5 relative overflow-hidden">
        <div className="absolute top-0 right-0 w-32 h-32 bg-accent-gold/5 rounded-full blur-2xl" />
        <div className="flex flex-col md:flex-row items-start md:items-center justify-between gap-4 relative z-10">
          <div className="space-y-1">
            <h4 className="text-xs font-black uppercase text-accent-gold tracking-[0.25em] flex items-center gap-2">
              ⚠️ {t('REGULATORY COLD STORAGE ACTIVE (LYA v3.0)', 'GEL RÉGLEMENTAIRE DU PROTOCOLE (LYA v3.0)')}
            </h4>
            <p className="text-[11px] text-on-surface-variant/80 font-medium leading-relaxed max-w-5xl text-justify uppercase tracking-wider">
              {t(
                'In compliance with LYA Protocol v3.0 (Regulatory-First Architecture), negotiable secondary market transactions (including exchange orders and instant settlement) are in a regulatory intermission phase. The terminal currently operates on static, sandboxed datasets for objective certification, LYA Score evaluation, and professional audit indexing purposes only.',
                "Conformément à la version v3.0 du protocole LYA (Regulatory-First), les transactions négociables du marché secondaire (incluant carnets d'ordres et règlements instantanés) sont actuellement suspendues dans l'attente d'agréments. Le terminal fonctionne actuellement sur des données statiques simulées à des fins exclusives de certification objective, d'évaluation de LYA Score et d'indexation d'audits professionnels."
              )}
            </p>
          </div>
          <span className="shrink-0 text-[10px] font-black text-accent-gold border border-accent-gold/30 px-3 py-1 uppercase tracking-widest bg-accent-gold/10">
            {t('PHASE 4 DEFERRED', 'PHASE 4 REPORTÉE')}
          </span>
        </div>
      </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
              {paginatedContracts.map(contract => (
                <ContractCard
                  ...
                  onTrade={(c, type) => { if (type === 'BUY') onNotify(t('Direct acquisitions are suspended during this regulatory phase (v3.0).', 'Les acquisitions directes sont suspendues durant cette phase réglementaire (v3.0).')); else onNotify(t('Secondary market transfers are paused under the regulatory-first protocol.', 'Les transferts de marché secondaire sont suspendus sous le protocole de conformité.')); }}
                  onToggleWatchlist={onToggleWatchlist}
                  isWatchlisted={watchlist.includes(contract.id)}
