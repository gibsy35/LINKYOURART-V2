...
export const HoldingsView: React.FC<{ onNotify: (msg: string) => void; userContracts?: any[]; onViewChange: (view: any) => void }> = ({ onNotify, userContracts = [], onViewChange }) => {
  ...
  return (
    <div className="space-y-6 pb-12 relative overflow-visible">
      <PageHeader 
        titleWhite={t('Sovereign', 'Registre')}
        titleAccent={t('Holdings', 'des Droits')}
        description={t('CERTIFICATE & VALUATION STATEMENT: OFFICIALLY REGISTERED ASSETS AND AUDITED LYA SCORES.', 'RELEVÉ DE CERTIFICATION ET VALORISATION : DROITS ENREGISTRÉS ET SCORES LYA CERTIFIÉS.')}
        ...
        <div className="flex flex-col md:flex-row items-start md:items-center justify-between gap-4 relative z-10">
          <div className="space-y-1">
            ...
            <p className="text-[11px] text-on-surface-variant/80 font-medium leading-relaxed max-w-5xl text-justify uppercase tracking-wider">
              {t(
                'In compliance with LYA Protocol v3.0 (Regulatory-First Architecture), negotiable secondary market transactions (including peer-to-peer exchanges and instant settlement) are in a regulatory intermission phase. The holdings displayed represent officially registered contractual rights and audited LYA Scores only, on static sandboxed datasets.',
                "Conformément à la version v3.0 du protocole LYA (Regulatory-First), les transactions négociables (incluant les échanges de pair à pair et règlements instantanés) sont suspendues dans l'attente d'agréments. Les avoirs affichés représentent exclusivement des droits contractuels officiellement enregistrés et des LYA Scores certifiés sur des jeux de données statiques."
              )}
            </p>
          </div>
          ...
          {holdings.length === 0 ? (
            <div className="bg-surface-low border border-white/5 p-16 md:p-24 text-center rounded-2xl flex flex-col items-center justify-center">
              ...
              <p className="text-[10px] md:text-xs text-on-surface-variant max-w-sm uppercase font-bold tracking-[0.2em] opacity-40 mb-8 leading-relaxed">
                {t('Your creative portfolio is currently empty. Visit the Registry to view available creative projects.', 'Votre portefeuille créatif est actuellement vide. Visitez le Registre pour voir les projets créatifs disponibles.')}
              </p>
              <button 
              ...
              >
                {t('Go to Registry', 'Aller au Registre')}
              </button>
            </div>
          ) : (
