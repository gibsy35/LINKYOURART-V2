...
export const AuthModal: React.FC<AuthModalProps> = ({ isOpen, onClose, onNotify, setUser }) => {
  ...
  const handleSetMode = (m: 'LOGIN' | 'SIGNUP') => {
    setMode(m);
    setError(null);
  };
  ...
  const handleGoogleAuth = async () => {
    ...
    try {
      ...
      const errMsg = err.message || t('AUTHENTICATION FAILED', "ÉCHEC DE L'AUTHENTIFICATION");
      setError(errMsg);
          ...
          <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: 20 }} className="fixed inset-0 flex items-center justify-center p-4 z-[401] pointer-events-none">
            <div className="bg-surface-dim border border-white/10 w-full max-w-lg pointer-events-auto p-8 relative overflow-hidden font-mono max-h-[90vh] overflow-y-auto">
              ...
              <div className="text-center mb-6">
                ...
                <h2 className="text-xl font-black text-white uppercase tracking-tighter mb-1">{t('LYA TERMINAL v3.0.0', 'TERMINAL LYA v3.0.0')}</h2>
              ...
              {error && (
                <div className="p-4 bg-red-500/10 border border-red-500/20 text-red-500 text-[10px] rounded mb-4">
                  {error.toLowerCase().includes('unauthorized-domain') ? (
                    <div className="space-y-3 text-left">
                      <div className="font-bold uppercase tracking-wider text-center text-red-400">
                        {t('Firebase Unauthorized Domain Error', 'Erreur de domaine Firebase non autorisé')}
                      </div>
                      <p className="text-gray-300 leading-relaxed font-sans normal-case">
                        {t(
                          'This domain is not authorized in your Firebase project settings for authentication. To enable Google Login for this domain:',
                          'Ce domaine n\'est pas autorisé dans les paramètres de votre projet Firebase pour l\'authentification. Pour activer la connexion Google :'
                        )}
                      </p>
                      <ol className="list-decimal list-inside space-y-1 text-gray-400 font-sans normal-case text-[9px]">
                        <li>
                          {t('Open the ', 'Ouvrez la ')}
                          <a href="https://console.firebase.google.com" target="_blank" rel="noopener noreferrer" className="text-primary-cyan underline hover:text-cyan-400">
                            Firebase Console
                          </a>
                        </li>
                        <li>{t('Navigate to Build > Authentication > Settings tab.', 'Allez dans Build > Authentication > onglet Settings.')}</li>
                        <li>{t('Scroll down to "Authorized domains".', 'Faites défiler jusqu\'à "Authorized domains".')}</li>
                        <li>
                          {t('Click "Add domain" and enter: ', 'Cliquez sur "Add domain" et saisissez : ')}
                          <code className="bg-white/10 px-1.5 py-0.5 rounded text-white font-mono select-all">
                    ...
                    <div className="font-black uppercase tracking-widest text-center">{error}</div>
                  )}
                </div>
              )}
