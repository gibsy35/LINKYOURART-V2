...
export const BreakingNewsTicker: React.FC<BreakingNewsTickerProps> = ({ items, onItemClick }) => {
  ...
  const defaultItems: NewsTickerItem[] = [
    { label: t('Breaking News', 'Flash Info'), content: t('LYA v3.0.0 — Nouvelle version prête', 'LYA v3.0.0 — New version ready'), type: 'DANGER', icon: <div className="w-1.5 h-1.5 bg-red-500/50 rounded-full animate-pulse shadow-[0_0_8px_rgba(239,68,68,0.4)]" /> },
    ...
    { label: t('New Version', 'Nouvelle Version'), content: t('Sovereign Creative Platform v3.0 Active', 'Plateforme Créative Souveraine v3.0 Active'), type: 'INFO' },
    { label: t('Professional Node', 'Nœud Professionnel'), content: t('Validation nodes fully synchronized for v3.0', 'Nœuds de validation synchronisés pour v3.0'), type: 'WARNING' }
  ];
