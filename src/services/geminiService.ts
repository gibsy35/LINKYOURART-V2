...
export const generateImage = async (description: string, style: string) => {
  ...
  if (!response.ok) {
    throw new Error(`Image Gen API failed with status ${response.status}`);
  }
  const data = await response.json();
  if (data && data.success && data.imageData) {
    return `data:image/png;base64,${data.imageData}`;
  }
  throw new Error("Invalid image response data format");
};
