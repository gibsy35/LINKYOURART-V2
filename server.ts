...
// Initialize Gemini AI lazily via Proxy
const aiRef = { client: null as GoogleGenAI | null };
const ai = new Proxy({} as GoogleGenAI, {
  get(target, prop, receiver) {
    ...
    return Reflect.get(aiRef.client, prop, receiver);
  }
});
...
// Initialize Stripe lazily via Proxy
const stripeRef = { client: null as Stripe | null };
const stripe = new Proxy({} as Stripe, {
  get(target, prop, receiver) {
    if (!stripeRef.client) {
      stripeRef.client = new Stripe(process.env.STRIPE_SECRET_KEY || 'sk_test_placeholder');
    }
    return Reflect.get(stripeRef.client, prop, receiver);
  }
});
...
async function startServer() {
  ...
  app.post('/api/gemini/generate-image', async (req, res) => {
    try {
      ...
      if (base64Data) {
        res.json({ success: true, imageData: base64Data });
      } else {
